...
<head>
  ...
  <title>Approved NQR List - NQR</title>
  ...
  <main id="main" class="main">
    ...
    <ul class="nav nav-tabs">
      <li class="nav-item">
        ...
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status = 2");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status == 2 ? 'active' : ''; ?>" aria-current="page"
          href="approveqa.php?status=2">
          Approved by Foreman<span style="font-size: smaller; opacity: 0.7;">
      ...
      <li class="nav-item">
        <?php
        // Melakukan query untuk menghitung jumlah status dengan nilai 2
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status = 3");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status == 3 ? 'active' : ''; ?>" href="approveqa.php?status=3">Approved
          by
      ...
      <li class="nav-item">
        <?php
        // Melakukan query untuk menghitung jumlah status dengan nilai 2
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status = 4");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status == 4 ? 'active' : ''; ?>" href="approveqa.php?status=4">Approved
          by
  ...
  <script>
    $(document).ready(function () {
      $('#myTable').DataTable({
        ...
        "lengthMenu": [5, 10, 15, 20], // Menentukan opsi tampilan jumlah entri per halaman
      });
    });
  </script>

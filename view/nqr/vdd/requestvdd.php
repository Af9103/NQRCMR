...
$status_vdd = isset($_GET['status_vdd']) ? $_GET['status_vdd'] : "1";
  ...
  <main id="main" class="main">
    ...
    <ul class="nav nav-tabs">
      <li class="nav-item">
        ...
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status_vdd = 1");
        ...
        <a class="nav-link <?php echo $status_vdd == 1 ? 'active' : ''; ?>" aria-current="page"
          href="requestvdd.php?status_vdd=1">PPC Finish<span style="font-size: smaller; opacity: 0.7;">
            (<?php echo $jumlahStatus; ?>)</span></a>
      </li>
      <li class="nav-item">
        <?php
        // Melakukan query untuk menghitung jumlah status dengan nilai 2
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status_vdd = 2");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status_vdd == 2 ? 'active' : ''; ?>"
          href="requestvdd.php?status_vdd=2">Requested<span style="font-size: smaller; opacity: 0.7;">
            (<?php echo $jumlahStatus; ?>)</span></a>
      </li>
    </ul>
    <!-- konten -->

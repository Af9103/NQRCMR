...
<body>
    ...
    if (isset($_POST['proses'])) {
        ...
        $result_approvespv = mysqli_query($koneksi, "UPDATE transaksi SET dt_spv_vdd='$dt_spv_vdd', nm_spv_vdd='$nm_spv_vdd', status_vdd='4', sts_spv_vdd='1' WHERE Id='$Id'");
        if ($result_approvespv) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan NQR! NQR dengan nomor $reg_no telah di approve oleh Supervisor VDD $nm_spv_vdd. Status menunggu approval Manager.";
            $flags = "queue";
    ...
    var message = "NQR dengan nomor ' . $reg_no . ' telah di-approve oleh Supervisor PPC (' . $nm_spv_vdd . '). Klik link ini untuk memeriksa NQR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        window.location.href = "ap-spv-vdd.php";
    });
</script>';
    ...
    if (isset($_POST['tolak'])) {
        ...
        // Eksekusi query update
        $result_rejectspv = mysqli_query($koneksi, "UPDATE transaksi SET dt_spv_vdd='$dt_spv_vdd', nm_spv_vdd='$nm_spv_vdd', remark_spv_vdd='$remark_spv_vdd', remark_vdd='$remark_spv_vdd', status_vdd='7',sts_spv_vdd='2' WHERE Id='$Id'");
        if ($result_rejectspv) {
            // Kirim notifikasi jika query update berhasil
            ...
            echo '<script>
    var no_reg_sanitized = "' . preg_replace("/[^a-zA-Z0-9]+/", "", $reg_no) . '";
    var message = "NQR dengan nomor ' . $reg_no . ' telah di reject oleh Supervisor VDD(' . $nm_spv_vdd . ').dengan alasan ' . $remark_spv_vdd . ' Klik link ini untuk memeriksa reg: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "error",
        title: "NQR Rejected",
        text: "NQR dengan nomor ' . $reg_no . ' telah di-reject oleh ' . $nm_spv_vdd . ' pada ' . $dt_spv_vdd . ' dengan remark ' . $remark_spv_vdd . '",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-spv-vdd.php";

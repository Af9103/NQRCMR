...
<body>
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="ap-fm-vdd.php">List NQR</a></li>
    ...
    if (isset($_POST['proses'])) {
        ...
        $result_approvefm = mysqli_query($koneksi, "UPDATE transaksi SET dt_fm_vdd='$dt_fm_vdd', nm_fm_vdd='$nm_fm_vdd', status_vdd='3', sts_fm_vdd='1' WHERE Id='$Id'");
        if ($result_approvefm) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan NQR! NQR dengan nomor $reg_no telah di approve oleh foreman VDD $nm_fm_vdd. Status menunggu approval supervisor.";
            $flags = "queue";
    ...
    var message = "NQR dengan nomor ' . $reg_no . ' telah di-approve oleh Foreman VDD (' . $nm_fm_vdd . '). Klik link ini untuk memeriksa NQR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "success",
        title: "NQR Approved",
        text: "NQR dengan nomor ' . $reg_no . ' telah di-approve oleh ' . $nm_fm_vdd . ' pada ' . $dt_fm_vdd . ' ",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-fm-vdd.php";
    });
</script>';
    ...
    if (isset($_POST['tolak'])) {
        ...
        // Eksekusi query update
        $result_rejectfm = mysqli_query($koneksi, "UPDATE transaksi SET dt_fm_vdd='$dt_fm_vdd', nm_fm_vdd='$nm_fm_vdd', remark_fm_vdd='$remark_fm_vdd', remark_vdd='$remark_fm_vdd', status_vdd='6',sts_fm_vdd='2' WHERE Id='$Id'");
        if ($result_rejectfm) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan NQR! NQR dengan nomor $reg_no telah di reject oleh foreman VDD $nm_fm_vdd dengan remark $remark_fm_vdd";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
            ...
            echo '<script>
    var no_reg_sanitized = "' . preg_replace("/[^a-zA-Z0-9]+/", "", $reg_no) . '";
    var message = "NQR dengan nomor ' . $reg_no . ' telah di reject oleh Foreman VDD(' . $nm_fm_vdd . ').dengan alasan ' . $remark_fm_vdd . ' Klik link ini untuk memeriksa reg: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "error",
        title: "NQR Rejected",
        text: "NQR dengan nomor ' . $reg_no . ' telah di-reject oleh ' . $nm_fm_vdd . ' pada ' . $dt_fm_vdd . ' dengan remark ' . $remark_fm_vdd . '",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-fm-vdd.php";

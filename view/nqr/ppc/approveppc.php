...
$status_ppc = isset($_GET['status_ppc']) ? $_GET['status_ppc'] : "3";
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Approved NQR List</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <ul class="nav nav-tabs">
      <li class="nav-item">
        ...
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status_ppc = 3");
        ...
        <a class="nav-link <?php echo $status_ppc == 3 ? 'active' : ''; ?>" aria-current="page"
          href="approveppc.php?status_ppc=3">Approved by Foreman <span style="font-size: smaller; opacity: 0.7;">
            (<?php echo $jumlahStatus; ?>)</span></a>
      </li>
      <li class="nav-item">
        <?php
        // Melakukan query untuk menghitung jumlah status dengan nilai 2
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status_ppc = 4");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status_ppc == 4 ? 'active' : ''; ?>" href="approveppc.php?status_ppc=4">Approved
          by
      ...
      <li class="nav-item">
        <?php
        // Melakukan query untuk menghitung jumlah status dengan nilai 2
        $queryJumlahData = mysqli_query($koneksi, "SELECT COUNT(*) AS jumlah FROM transaksi WHERE YEAR(dt_op_qa) = $currentYear AND status_ppc = 5");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status_ppc == 5 ? 'active' : ''; ?>" href="approveppc.php?status_ppc=5">Approved
          by

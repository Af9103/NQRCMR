...
if ($query) {
    ...
    if ($user_data) {
        ...
        $dt_spv_ppc = date("Y-m-d H:i:s"); // Menggunakan format "YYYY-MM-DD HH:MM:SS" untuk tanggal dan waktu saat ini
    }
} else {
...
<body>
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="ap-spv-ppc.php">List NQR</a></li>
                        ...
                        <div class="card-body">
                            ...
                            <form method="post" id="myForm">
                                ...
                                <div class="row mb-3" style="display: none;">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="nm_spv_ppc" name="nm_spv_ppc"
                        ...
                        <form class="row g-3" action="" method="post" class="modal-dialog" role="document">
                            <div class="col-md-12">
                                <div class="form-floating">
                                    <textarea class="form-control" style="height: 150px" id="remark_spv_ppc"
                                        name="remark_spv_ppc"></textarea>
                            ...
                            <div class="row mb-3" style="display: none;">
                                <label for="inputText" class="col-sm-4 col-form-label">Nama</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="nm_spv_ppc" name="nm_spv_ppc" readonly
    ...
    if (isset($_POST['proses'])) {
        ...
        date_default_timezone_set("Asia/Bangkok"); // Set zona waktu ke Asia/Bangkok
        $dt_spv_ppc = date("Y-m-d H:i:s"); // Menggunakan format "YYYY-MM-DD HH:MM:SS" untuk tanggal dan waktu saat ini
        $nm_spv_ppc = $_POST['nm_spv_ppc'];

        // Eksekusi query update
        $result_approvespv = mysqli_query($koneksi, "UPDATE transaksi SET dt_spv_ppc='$dt_spv_ppc', nm_spv_ppc='$nm_spv_ppc', status_ppc='4', sts_spv_ppc='1' WHERE Id='$Id'");
        if ($result_approvespv) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan NQR! NQR dengan nomor $reg_no telah di approve oleh Supervisor PPC $nm_spv_ppc. Status menunggu approval Manager.";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
                            WHERE ct_users.golongan = 4 AND ct_users.acting = 1 AND dept = 'PPC'";
    ...
    var message = "NQR dengan nomor ' . $reg_no . ' telah di approve oleh Supervisor PPC(' . $nm_spv_ppc . '). Klik link ini untuk memeriksa NQR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "success",
        title: "NQR Approve",
        text: "NQR dengan nomor ' . $reg_no . ' telah di-approve oleh ' . $nm_spv_ppc . ' pada ' . $dt_spv_ppc . ' ",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-spv-ppc.php";
    });
</script>';
    ...
    if (isset($_POST['tolak'])) {
        ...
        $nm_spv_ppc = $_POST['nm_spv_ppc'];
        $remark_spv_ppc = input($_POST['remark_spv_ppc']); // Ambil nilai remark dari formulir modal
        date_default_timezone_set("Asia/Bangkok"); // Set zona waktu ke Asia/Bangkok
        $dt_spv_ppc = date("Y-m-d H:i:s"); // Menggunakan format "YYYY-MM-DD HH:MM:SS" untuk tanggal dan waktu saat ini
    
        // Eksekusi query update
        $result_rejectspv = mysqli_query($koneksi, "UPDATE transaksi SET dt_spv_ppc='$dt_spv_ppc', nm_spv_ppc='$nm_spv_ppc', remark_spv_ppc='$remark_spv_ppc',  remark_ppc='$remark_spv_ppc', status_ppc='7',sts_spv_ppc='2' WHERE Id='$Id'");
        if ($result_rejectspv) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan NQR! NQR dengan nomor $reg_no telah di reject oleh Supervisor PPC $nm_spv_ppc dengan remark $remark_spv_ppc";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
            ...
            echo '<script>
    var no_reg_sanitized = "' . preg_replace("/[^a-zA-Z0-9]+/", "", $reg_no) . '";
    var message = "NQR dengan nomor ' . $reg_no . ' telah di reject oleh Supervisor PPC(' . $nm_spv_ppc . ').dengan alasan ' . $remark_spv_ppc . ' Klik link ini untuk memeriksa NQR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "error",
        title: "NQR Rejected",
        text: "NQR dengan nomor ' . $reg_no . ' telah di-reject oleh ' . $nm_spv_ppc . ' pada ' . $dt_spv_ppc . ' dengan remark ' . $remark_spv_ppc . '",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-spv-ppc.php";

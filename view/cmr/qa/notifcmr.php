    ...
    if (mysqli_num_rows($result_phone) > 0) {
      ...
      while ($phone_row = mysqli_fetch_assoc($result_phone)) {
        ...
        if (!mysqli_query($koneksi3, $query_insert_notif)) {
          die("Error inserting notification for phone number $phone_number: " . mysqli_error($koneksi3));
        }
      }
...
<body>
  ...
  <main id="main" class="main">
    ...
    <ul class="nav nav-tabs">
      ...
      <li class="nav-item">
        <a class="nav-link active" href="notifcmr.php">CMR<span style="font-size: smaller; opacity: 0.7;"></span></a>
      </li>
    </ul>
          ...
          <table class="table table-hover text-center" id="ppcTable">
            ...
            <tbody>
              ...
              while ($user_data = mysqli_fetch_array($queryKasir)) {
                ...
                echo '<td><button type="button" class="btn btn-warning btn-sm" onclick="sendWhatsAppMessage(\'' . $user_data['cmr_no'] . '\', \'' . $user_data['approval_role'] . '\', \'' . formatDelay($user_data['delay']) . '\')" style="background-color: red;">Click Me</button></td>';
                echo "</tr>";
              }
              ?>
          </table>
  ...
  <script>
    $(document).ready(function () {
      $('#ppcTable').DataTable({
        ...
        "order": [[1]] // Mengurutkan berdasarkan kolom pertama secara menaik
      });
    });
  </script>

...
$queryKasir = mysqli_query($koneksi3, "SELECT * FROM transaksi t JOIN status_cmr s ON t.Id = s.Id WHERE YEAR(s.dt_op_qa) = $currentYear AND s.sts_cmr IN (1,9) ORDER BY FIELD(s.sts_cmr, 1,9)");
...
<body>
  ...
  <main id="main" class="main">
    ...
    <ul class="nav nav-tabs">
      <li class="nav-item">
        <a class="nav-link" aria-current="page" href="../../nqr/qa/ap-fm.php">
          NQR<span style="font-size: smaller; opacity: 0.7;"> </span>
        </a>
      </li>

      <li class="nav-item">
        <a class="nav-link active" href="ap-fm-cmr.php">CMR<span style="font-size: smaller; opacity: 0.7;"></span></a>
      </li>
    </ul>
          ...
          <table class="table table-hover text-center" id="dasborTable">
            ...
            <tbody>
              ...
              while ($user_data = mysqli_fetch_array($queryKasir)) {
                ...
                echo '<strong>feedback</strong>: ' . $feedback_qa;
                echo '</div>';
                echo '</div>';
                ...
                echo "<td><a href='ap-fm-cmr-detail.php?Id={$user_data['Id']}' class='btn btn-info'>

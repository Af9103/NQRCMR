...
$query = mysqli_query($koneksi3, "SELECT t.*, s.sts_cmr, s.nm_op_qa, s.status_op_qa, s.dt_op_qa
    FROM transaksi t
...
<body>
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="ap-spv-cmr.php">List CMR</a></li>
                            ...
                            <form method="post" enctype="multipart/form-data">
                                ...
                                <div class="preview-container">
                                    ...
                                    <div class="row mb-3" style="display: none;">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="nm_spv_qa" name="nm_spv_qa"
                                ...
                                <form class="row g-3" action="" method="post" class="modal-dialog" role="document">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="button" class="btn btn-danger custom-button" aria-label="Close"
                                            onclick="document.getElementById('remark_spv_qa').value='';">Reset</button>
    ...
    if (isset($_POST['proses'])) {
        ...
        $result_approvespv = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_spv_qa='$dt_spv_qa', nm_spv_qa='$nm_spv_qa', sts_cmr='3',status_spv_qa='1' WHERE Id='$Id'");
        if ($result_approvespv) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan CMR! CMR dengan nomor $cmr_no telah di approve oleh Supervisor QA $nm_spv_qa. Status menunggu approval Manager.";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
                            WHERE ct_users.golongan = 4 AND ct_users.acting = 1 AND dept = 'QA'";
    ...
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di-Approve oleh Supervisor QA (' . $nm_spv_qa . '). Klik link ini untuk memeriksa CMR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-approve oleh ' . $nm_spv_qa . ' pada ' . $dt_spv_qa . ' ",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-spv-cmr.php";
    });
    </script>';
        } else {
    ...
    if (isset($_POST['tolak'])) {
        ...
        // Eksekusi query update
        $result_rejectspv = "UPDATE status_cmr SET dt_spv_qa='$dt_spv_qa', nm_spv_qa='$nm_spv_qa', sts_cmr='10',status_spv_qa='2', remark_spv_qa='$remark_spv_qa', remark='$remark_spv_qa' WHERE Id='$Id'";

        if ($result_rejectspv) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan NQR! NQR dengan nomor $cmr_no telah di reject oleh Supervisor QA $nm_spv_qa dengan remark $remark_spv_qa";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
    ...
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di reject oleh Supervisor(' . $nm_spv_qa . ').dengan alasan ' . $remark_spv_qa . ' Klik link ini untuk memeriksa CMR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "error",
        title: "NQR Rejected",
        text: "NQR dengan nomor ' . $cmr_no . ' telah di-reject oleh ' . $nm_spv_qa . ' pada ' . $dt_spv_qa . ' dengan remark ' . $remark_spv_qa . '",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-spv-cmr.php";

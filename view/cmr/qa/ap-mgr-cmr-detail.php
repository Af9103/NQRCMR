...
<body>
                  ...
                  <div class="col-sm-8">
                    <input type="text" name="feedback" class="form-control" value="<?php echo $feedback ?>">
                                    ...
                                    <div class="row mb-3" style="display: none;">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="nm_mgr_qa" name="nm_mgr_qa"
    ...
    if (isset($_POST['proses'])) {
        ...
        $result_approvemgr = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_mgr_qa='$dt_mgr_qa', nm_mgr_qa='$nm_mgr_qa', sts_cmr='4',status_mgr_qa='1' WHERE Id='$Id'");
    ...
    Swal.fire({
        ...
        icon: "success",
        title: "CMR approved",
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-approve oleh ' . $nm_mgr_qa . ' pada ' . $dt_mgr_qa . ' ",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-mgr-cmr.php";
    });
    </script>';
        } else {
    ...
    if (isset($_POST['tolak'])) {
        $nm_mgr_qa = $_POST['nm_mgr_qa'];
        $remark_mgr_qa = mysqli_real_escape_string($koneksi3, $_POST['remark_mgr_qa']); // Sanitize input for security
        date_default_timezone_set("Asia/Bangkok"); // Set zona waktu ke Asia/Bangkok
        $dt_mgr_qa = date("Y-m-d H:i:s"); // Menggunakan format "YYYY-MM-DD HH:MM:SS" untuk tanggal dan waktu saat ini
    
        // Eksekusi query update
        $result_rejectmgr = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_mgr_qa='$dt_mgr_qa', nm_mgr_qa='$nm_mgr_qa', sts_cmr='11',status_mgr_qa='2', remark_mgr_qa='$remark_mgr_qa', remark='$remark_mgr_qa' WHERE Id='$Id'");
        ...
        Swal.fire({
            ...
            icon: "error",
            title: "NQR Rejected",
            text: "NQR dengan nomor ' . $cmr_no . ' telah di-reject oleh ' . $nm_mgr_qa . ' pada ' . $dt_mgr_qa . ' dengan remark ' . $remark_mgr_qa . '",
            showConfirmButton: false,
            timer: 2000
        }).then(() => {
            window.location.href = "ap-mgr-cmr.php";

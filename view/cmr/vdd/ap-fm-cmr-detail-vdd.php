...
if (!isset($_SESSION["username"]) || !isset($_SESSION["dept"]) || $_SESSION["dept"] !== "VDD" || !in_array($_SESSION["role"], ["FMVDD", "SPVVDD", "MGRVDD"])) {
...
<head>
    ...
    <title>Approval Foreman - CMR</title>
...
<body>
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="ap-fm-cmr-vdd.php">List NQR</a></li>
                        ...
                        <div class="card-body">
                            ...
                            <form method="post" id="myForm">
                                ...
                                <div class="row mb-3" style="display: none;">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="nm_fm_vdd" name="nm_fm_vdd" readonly
                        ...
                        <form class="row g-3" action="" method="post" class="modal-dialog" role="document">
                            <div class="col-md-12">
                                <div class="form-floating">
                                    <textarea class="form-control" style="height: 150px" id="remark_fm_vdd"
                                        name="remark_fm_vdd"></textarea>
    ...
    if (isset($_POST['proses'])) {
        ...
        date_default_timezone_set("Asia/Bangkok"); // Set zona waktu ke Asia/Bangkok
        $dt_fm_vdd = date("Y-m-d H:i:s"); // Menggunakan format "YYYY-MM-DD HH:MM:SS" untuk tanggal dan waktu saat ini
        $nm_fm_vdd = $_POST['nm_fm_vdd'];

        // Eksekusi query update
        $result_approvefm = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_fm_vdd='$dt_fm_vdd', nm_fm_vdd='$nm_fm_vdd', sts_cmr_vdd='3', status_fm_vdd='1' WHERE Id='$Id'");
        if ($result_approvefm) {

            $message = "Pemberitahuan CMR! CMR dengan nomor $cmr_no telah di-approve oleh Foreman VDD $nm_fm_vdd. Status menunggu approval supervisor.";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
                            WHERE ct_users.golongan = 4 AND ct_users.acting = 2 AND dept = 'VDD'";
    ...
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di-approve oleh Foreman VDD (' . $nm_fm_vdd . '). Klik link ini untuk memeriksa NQR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "success",
        title: "CMR Approved",
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-approve oleh ' . $nm_fm_vdd . ' pada ' . $dt_fm_vdd . ' ",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-fm-cmr-vdd.php";
    });
</script>';
    ...
    if (isset($_POST['tolak'])) {
        ...
        $nm_fm_vdd = $_POST['nm_fm_vdd'];
        $remark_fm_vdd = input($_POST['remark_fm_vdd']); // Ambil nilai remark dari formulir modal
        date_default_timezone_set("Asia/Bangkok"); // Set zona waktu ke Asia/Bangkok
        $dt_fm_vdd = date("Y-m-d H:i:s"); // Menggunakan format "YYYY-MM-DD HH:MM:SS" untuk tanggal dan waktu saat ini
    
        // Eksekusi query update
        $result_rejectfm = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_fm_vdd='$dt_fm_vdd', nm_fm_vdd='$nm_fm_vdd', remark_fm_vdd='$remark_fm_vdd', remark_vdd='$remark_fm_vdd', sts_cmr_vdd='6',status_fm_vdd='2' WHERE Id='$Id'");
        if ($result_rejectfm) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan CMR! CMR dengan nomor $cmr_no telah di reject oleh foreman VDD $nm_fm_vdd dengan remark $remark_fm_vdd";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
            ...
            echo '<script>
    var no_reg_sanitized = "' . preg_replace("/[^a-zA-Z0-9]+/", "", $cmr_no) . '";
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di reject oleh Foreman VDD(' . $nm_fm_vdd . ').dengan alasan ' . $remark_fm_vdd . ' Klik link ini untuk memeriksa CMR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "error",
        title: "CMR Rejected",
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-reject oleh ' . $nm_fm_vdd . ' pada ' . $dt_fm_vdd . ' dengan remark ' . $remark_fm_vdd . '",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-fm-cmr-vdd.php";

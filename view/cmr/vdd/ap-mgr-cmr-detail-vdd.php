...
<body>
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="ap-mgr-cmr-vdd.php">List NQR</a></li>
    ...
    if (isset($_POST['proses'])) {
        ...
        $result = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_mgr_vdd='$dt_mgr_vdd', nm_mgr_vdd='$nm_mgr_vdd', sts_cmr_vdd='5', status_mgr_vdd='1' WHERE Id='$Id'");
        if ($result) {
            // Jika berhasil, redirect ke 'ap-mgr.php'
            echo '<script>
    var no_reg_sanitized = "' . preg_replace("/[^a-zA-Z0-9]+/", "", $cmr_no) . '";
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di-approve oleh Supervisor VDD (' . $nm_mgr_vdd . '). Klik link ini untuk memeriksa NQR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "success",
        title: "CMR Approved",
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-approve oleh ' . $nm_mgr_vdd . ' pada ' . $dt_mgr_vdd . ' ",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-mgr-cmr-vdd.php";
    });
</script>';
    ...
    if (isset($_POST['tolak'])) {
        ...
        // Eksekusi query update
        $result_rejectmgr = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_mgr_vdd='$dt_mgr_vdd', nm_mgr_vdd='$nm_mgr_vdd', remark_mgr_vdd='$remark_mgr_vdd', remark_vdd='$remark_mgr_vdd', sts_cmr_vdd='8',status_mgr_vdd='2' WHERE Id='$Id'");
        if ($result_rejectmgr) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan CMR! CMR dengan nomor $cmr_no telah di reject oleh Manager VDD $nm_mgr_vdd dengan remark $remark_mgr_vdd";
            $flags = "queue";
            ...
            echo '<script>
    var no_reg_sanitized = "' . preg_replace("/[^a-zA-Z0-9]+/", "", $cmr_no) . '";
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di reject oleh Manager VDD(' . $nm_mgr_vdd . ').dengan alasan ' . $remark_mgr_vdd . ' Klik link ini untuk memeriksa CMR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "error",
        title: "CMR Rejected",
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-reject oleh ' . $nm_mgr_vdd . ' pada ' . $dt_mgr_vdd . ' dengan remark ' . $remark_mgr_vdd . '",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-mgr-cmr-vdd.php";

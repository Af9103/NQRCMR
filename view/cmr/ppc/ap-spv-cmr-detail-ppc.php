...
<body>
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="ap-spv-cmr-ppc.php">List CMR</a></li>
                    <li class="breadcrumb-item active">Approval by Supervisor</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
    ...
    if (isset($_POST['proses'])) {
        ...
        $result_approvespv = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_spv_ppc='$dt_spv_ppc', nm_spv_ppc='$nm_spv_ppc', sts_cmr_ppc='4', sts_spv_ppc='1' WHERE Id='$Id'");
        if ($result_approvespv) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan CMR! CMR dengan nomor $cmr_no telah di approve oleh Supervisor PPC $nm_spv_ppc. Status menunggu approval Manager.";
            $flags = "queue";
    ...
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di approve oleh supervisor PPC(' . $nm_spv_ppc . '). Klik link ini untuk memeriksa NQR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "success",
        title: "CMR Approved",
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-approve oleh ' . $nm_spv_ppc . ' pada ' . $dt_spv_ppc . ' ",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-spv-cmr-ppc.php";
    });
</script>';
        } else {
    ...
    if (isset($_POST['tolak'])) {
        ...
        // Eksekusi query update
        $result_rejectspv = mysqli_query($koneksi3, "UPDATE status_cmr SET dt_fm_ppc='$dt_spv_ppc', nm_spv_ppc='$nm_spv_ppc', remark_spv_ppc='$remark_spv_ppc', remark_ppc='$remark_spv_ppc', sts_cmr_ppc='7',sts_spv_ppc='2' WHERE Id='$Id'");
        if ($result_rejectspv) {
            // Kirim notifikasi jika query update berhasil
            $message = "Pemberitahuan CMR! CMR dengan nomor $cmr_no telah di reject oleh Supervisor PPC $nm_spv_ppc dengan remark $remark_spv_ppc";
            $flags = "queue";
            $query_phone = "SELECT no_hp FROM isd 
                            LEFT JOIN ct_users ON ct_users.npk = isd.npk 
    ...
    var message = "CMR dengan nomor ' . $cmr_no . ' telah di reject oleh Supervsior PPC(' . $nm_spv_ppc . ').dengan alasan ' . $remark_spv_ppc . ' Klik link ini untuk memeriksa CMR: http://e-learning.stmi.ac.id/mhs/login";
    ...
    Swal.fire({
        ...
        icon: "error",
        title: "CMR Rejected",
        text: "CMR dengan nomor ' . $cmr_no . ' telah di-reject oleh ' . $nm_spv_ppc . ' pada ' . $dt_spv_ppc . ' dengan remark ' . $remark_spv_ppc . '",
        showConfirmButton: false,
        timer: 2000
    }).then(() => {
        window.location.href = "ap-spv-cmr-ppc.php";

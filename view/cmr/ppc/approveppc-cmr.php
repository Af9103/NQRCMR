...
$status_ppc = isset($_GET['sts_cmr_ppc']) ? $_GET['sts_cmr_ppc'] : "3";

// Query untuk mengambil data kota sesuai status yang dipilih
$query1 = mysqli_query($koneksi3, "SELECT * FROM transaksi t INNER JOIN status_cmr s ON t.Id = s.Id WHERE YEAR(s.dt_op_qa) = $currentYear AND s.sts_cmr_ppc IN ($status_ppc)");
...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Approved CMR List</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="dasborcmr_ppc.php">Home</a></li>
          <!-- <li class="breadcrumb-item"><a href="#">#</a></li> -->
          <li class="breadcrumb-item active">CMR Approved</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <ul class="nav nav-tabs">
      <li class="nav-item">
        ...
        $queryJumlahData = mysqli_query($koneksi3, "SELECT COUNT(*) AS jumlah FROM status_cmr WHERE YEAR(dt_op_qa) = $currentYear AND sts_cmr_ppc = 3");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status_ppc == 3 ? 'active' : ''; ?>" aria-current="page"
          href="apppc-cmr.php?sts_cmr_ppc=3">Approved by Foreman <span style="font-size: smaller; opacity: 0.7;">
            (<?php echo $jumlahStatus; ?>)</span></a>
      </li>
      <li class="nav-item">
        <?php
        // Melakukan query untuk menghitung jumlah status dengan nilai 3
        $queryJumlahData = mysqli_query($koneksi3, "SELECT COUNT(*) AS jumlah FROM status_cmr WHERE YEAR(dt_op_qa) = $currentYear AND sts_cmr_ppc = 4");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status_ppc == 4 ? 'active' : ''; ?>" href="apppc-cmr.php?sts_cmr_ppc=4">Approved
          by SPV<span style="font-size: smaller; opacity: 0.7;">
            (<?php echo $jumlahStatus; ?>)</span></a>
      </li>
      <li class="nav-item">
        <?php
        // Melakukan query untuk menghitung jumlah status dengan nilai 2
        $queryJumlahData = mysqli_query($koneksi3, "SELECT COUNT(*) AS jumlah FROM status_cmr WHERE YEAR(dt_op_qa) = $currentYear AND sts_cmr_ppc = 5");
        $row = mysqli_fetch_assoc($queryJumlahData);
        $jumlahStatus = $row['jumlah'];
        ?>
        <a class="nav-link <?php echo $status_ppc == 5 ? 'active' : ''; ?>" href="apppc-cmr.php?sts_cmr_ppc=5">Approved
          by Manager<span style="font-size: smaller; opacity: 0.7;"> (<?php echo $jumlahStatus; ?>)</span></a>
      </li>
    </ul>
    <!-- konten -->
    <div class="card">
      <div class="card-body">
        <h5 class="card-title">Apporved <span>| NQR</span></h5>
...
// echo '</div>';
              ...
              $csSql = "SELECT * FROM transaksi t JOIN status_cmr s JOIN catatan c ON t.Id = s.Id = c.Id WHERE t.Id = '{$user_data['Id']}'";
